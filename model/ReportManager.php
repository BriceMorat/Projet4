...
class ReportManager extends Manager {
	...
	**/
	...
	public function getReports() {
		$db = $this->dbConnect();
		$reports = $db->query('SELECT COUNT(*) AS reports_nb, comment_id, author, content, DATE_FORMAT(date_comments, "%d/%m/%Y %H:%i") AS date_c FROM report INNER JOIN comment ON report.comment_id = comment.id GROUP BY comment_id HAVING reports_nb >= 2 ORDER BY reports_nb DESC');
